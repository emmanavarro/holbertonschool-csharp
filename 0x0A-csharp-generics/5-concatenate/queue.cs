...
class Queue<T> {
  ...
  /// <summary> Print the nodes </summary>
  public void Print() {
    if (this.head == null) {
      Console.WriteLine("Queue is empty");
    }
    ...
    Node actual;
    for (actual = this.head; actual != null; actual = actual.next) {
      Console.WriteLine(actual.value);
    }
  }

  /// <summary> Concatenate all the string and char nodes</summary>
  ...
  public string Concatenate() {
    ...
    actual = this.head;
    while (actual != null) {
      concatNode += actual.value.ToString();
      if (typeof(T) == typeof(string) && actual.next != null) {
        concatNode += " ";
      }
      actual = actual.next;
    }
